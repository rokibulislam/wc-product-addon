...
abstract class Base_Field {
	...
	public function print_label( $field, $form_id = 0 ) {
		?>
		<div class="prafe-label"> <label for="<?php echo isset( $field['name'] ) ? esc_attr( $field['name'] ) . '_' . esc_attr( $form_id ) : 'cls'; ?>">
	...
	public function print_list_attributes( $field ) {
		...
		printf(
			'class="prafe-el prafe-%s%s%s" data-label="%s" data-errormessage="%s"',
			esc_attr( $el_name ),
			esc_attr( $class_name ),
			esc_attr( $field_size ),
			esc_attr( $label ),
	...
	public function help_text( $field ) {
		...
		?>
		<span class="prafe-help"><?php echo esc_attr( $field['help'] ); ?></span>
		<?php
	}
	...
	public function prepare_entry( $field, $post_data = array() ) {
		...
		if ( is_array( $value ) ) {
			$entry_value = implode( PRAEF_SEPARATOR, $value );
		} else {
	...
	public function conditional_logic( $form_field, $form_id ) {
		if ( ! isset( $form_field['prafe_cond']['condition_status'] ) || $form_field['prafe_cond']['condition_status'] !== 'yes' ) {
			return;
		}

		$cond_inputs                     = $form_field['prafe_cond'];
		...
		if ( $cond_inputs['condition_status'] === 'yes' ) {
			...
			$script = "prafe_conditional_items.push({$condition});";
			wp_add_inline_script( 'prafe-frontend', $script );
		?>
		<?php
	}
}
